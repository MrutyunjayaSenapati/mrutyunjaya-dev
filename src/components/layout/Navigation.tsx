...
export default function Navigation() {
  ...
  const handleMobileNavClick = (e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    ...
    // Allow body overflow lock to clear, then smooth scroll to section
    setTimeout(() => {
        ...
        >
          ...
          <div className="flex items-center gap-2 md:hidden">
            <button
              ...
              className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-md border border-border text-text-secondary transition-colors hover:text-text"
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.nav
              ...
              className="overflow-y-auto max-h-[85vh] border-t border-border bg-surface/98 backdrop-blur-xl md:hidden shadow-2xl"
            >
              <div className="flex flex-col gap-1 px-4 pb-8 pt-4">
                {navItems.map((item, idx) => (
                  <motion.a
                    ...
                    className="flex items-baseline justify-between border-b border-border/60 py-3.5 font-display text-2xl font-medium text-text cursor-pointer hover:text-accent transition-colors"
                  >
                    {item.label}
                ...
                <a
                  ...
                  className="mt-4 inline-flex items-center justify-center gap-2 rounded-md bg-accent py-3 font-mono text-xs font-medium uppercase tracking-[0.08em] text-accent-ink"
                >
                  Resume
